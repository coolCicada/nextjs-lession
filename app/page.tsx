...
const features = [
  ...
  },
  {
    ...
    desc: '查看组合仓位与市场风向',
    icon: '☰',
    accent: 'from-amber-400/20 to-sky-300/10',
    border: 'border-amber-200/70 dark:border-amber-500/20',
    text: 'text-amber-600 dark:text-amber-300',
  },
  {
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h1 className="sm:text-5xl text-3xl font-semibold tracking-[-0.04em] text-slate-950 dark:text-white">
                OpenClaw
              </h1>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {features.map((feature, index) => (
                ...
                >
                  <Link
                    href={feature.href}
                    className={`group relative block overflow-hidden rounded-[24px] border ${feature.border} bg-gradient-to-br ${feature.accent} dark:hover:border-white/12 p-5 transition duration-200 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_12px_30px_rgba(15,23,42,0.06)] dark:hover:shadow-[0_16px_36px_rgba(0,0,0,0.22)]`}
                  >
                    <div className="bg-white/72 dark:bg-slate-950/18 absolute inset-0" />
                    <div className="relative">
                      <div
                        className={`mb-6 inline-flex h-12 w-12 items-center justify-center rounded-2xl border border-slate-200/80 bg-white text-2xl shadow-sm dark:border-white/10 dark:bg-white/10 ${feature.text}`}
                      >
                        {feature.icon}
                      </div>
                      ...
                      <div
                        className={`mt-5 text-sm font-medium ${feature.text}`}
                      >
                        进入 →
                      </div>
                    </div>
            ...
            <GlassPanel className="p-5 sm:p-6">
              ...
              <div className="mt-4 space-y-4">
                ...
                <div className="rounded-2xl border border-slate-200/80 bg-white p-4 text-sm leading-6 text-slate-500 dark:border-white/10 dark:bg-white/5 dark:text-slate-400">
                  这版首页把视觉语言往 Linear / Raycast / Vercel
                  的方向收了：更克制、更清爽、留白更稳定。
                </div>
              </div>
            </GlassPanel>
