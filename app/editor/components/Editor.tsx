...
const blocks = {
  paragraph: {
    render: (data: Block) => (
      <div dangerouslySetInnerHTML={{ __html: data.text }} className="prose" />
    ),
    edit: (
      data: Block,
      ...
      <div className="w-full rounded border p-2">
        ...
        <div
          ...
          onBlur={(e) =>
            setTimeout(() => onChange((e.target as HTMLElement).innerHTML), 100)
          }
        />
      </div>
    ),
...
const DraggableBlock: React.FC<{
  ...
  return (
    <div
      ...
      className={`mb-4 min-h-[100px] cursor-pointer rounded border bg-white p-4 shadow-sm
...
const Editor: React.FC = () => {
  const [content, setContent] = useState<Block[]>([
    { id: uuidv4(), type: 'header', text: 'Welcome to Custom Editor!' },
    {
      id: uuidv4(),
      type: 'paragraph',
      text: '<p>Type your <b>content</b> <i>here</i>...</p>',
    },
  ]);
  ...
  const handleChange = (id: string, text: string) => {
    setContent(
      content.map((block) => (block.id === id ? { ...block, text } : block)),
    );
  };
